...
import { GetAppVersion, MssqlQuery, PostgresQuery, PostgresGetDatabases } from '../wailsjs/go/main/App'
...
// Default connection settings for each database type
...
const data = reactive({
  appVersion: '',
  dbType: dbTypes[0],
  server: defaultSettings.mssql.server,
  dbs: [] as string[],
  db: '',
  username: defaultSettings.mssql.username,
  password: defaultSettings.mssql.password,
  query: defaultSettings.mssql.query,
  columns: [] as string[],
...
// Watch for database type changes and update default settings
watch(() => data.dbType, (newType) => {
  ...
  // Clear the database list and selection when switching types
  data.dbs = [];
  data.db = '';
  
  // Update the example query
  if (data.query === otherDefaults.query) {
    data.query = defaults.query;
  }
}, { immediate: false });

const serverPlaceholder = computed(() => {
  return data.dbType.value === 'mssql' ? 'server\\instance,port' : 'hostname:port';
});
...
const get_dbs = async () => {
  ...
  try {
    if (data.dbType.value === 'mssql') {
      ...
      // PostgreSQL
      const databases = await PostgresGetDatabases(data.server, data.username, data.password);
      if (requestId != data.requestId) return;
      data.dbs = databases;
      if (data.dbs.length >= 1) {
        data.db = data.dbs[0];
...
const execute = async () => {
  ...
  try {
    let r;
    if (data.dbType.value === 'mssql') {
      r = await MssqlQuery(data.server, data.username, data.password, data.db, data.query);
    } else {
      r = await PostgresQuery(data.server, data.username, data.password, data.db, data.query);
    }
...
<template>
  <div class="top" @keydown="(e) => (e.code == 'F5') && execute()">
    <div class="flex">
      ...
      <Dropdown v-model="data.dbType" :options="dbTypes" optionLabel="name" class="w-8rem mr-2" @change="get_dbs" />
      <InputText v-model="data.server" class="flex-1 min-w-0" :title="data.dbType.value === 'mssql' ? 'server\\instance,port' : 'hostname:port'" @change="get_dbs" />
